...
class DecoderRNN(nn.Module):
    ...
    def sample(self, inputs, states=None, max_len=20):
        ...
        while (cur_index < max_len) and (predicted_word != 1):

            # First, feed the inputs to the network first
            ...
            # Convert the output back to a number before appending
            # it to the predicted sequence
            predicted_word = word.item()
            predicted_sentence.append(predicted_word)
            
            # This predicted word will be an input of LSTM in the next time step
